	...
	if is_network_master():
		...
		if !skip:
			...
			if Input.is_action_just_pressed("weapon_drop") && weapon_helper.handedWeapon.droppable == true:
				...
				var dir = Vector2(-self.rotation_degrees.y, rotation_helper.rotation_degrees.x)
		...
		# ----------------------------------
		rset_id(1, "puppet_dir", dir)
		rset_id(1, "puppet_vel", vel)
...
func _input(event):
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED and is_network_master():
		rpc_unreliable_id(1, "rotate_on_mouse_input", event.relative.x, event.relative.y)
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED && is_network_master():
		if Input.is_action_pressed("weapon_shoot"):
			weapon_helper.rpc_unreliable_id(1, 'shoot')
		if Input.is_action_pressed("weapon_reload"):
			weapon_helper.rpc_unreliable_id(1, 'reload')
		if !Input.is_action_pressed("weapon_shoot"):
			#rpc_unreliable_id(1, 'weapon_helper.decrease_latest_recoil')
			pass
